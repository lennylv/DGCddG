...
from scipy.stats import kendalltau as pearsonr
...
items = np.load('./capri_t55.npy')
# items2 = np.load('../capri_56/itemlist.npy')
y = np.load('./itemlist.npy')[:,-1].astype(float)
y3 = y
y1 = np.load('../capri_t56/itemlist.npy')[:,-1].astype(float)
...
ssipe = readfromtxt('./set_2.txt')
ssipe3 = ssipe
ssipe2 = readfromtxt('../capri_t56/set_2.txt')
ssipe = np.concatenate([ssipe, ssipe2])
dgc = np.load('../prediction_2/capri_t55.npy')
dgc3 = dgc
dgc2 = np.load('../capri_t56/capri_t56.npy')
dgc = np.concatenate([dgc, dgc2])
print(pearsonr(y3, dgc3)[0])
print(pearsonr(y1, dgc2)[0])
if len(favor)>1:
