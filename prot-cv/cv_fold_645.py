...
parser.add_argument('--validation', type=int, default=0, help='device id')
...
for train_index, test_index in split_folds:
    ...
    if args.validation == 1:
        length = len(train_index)// 10
        train_index, test_index = train_index[:-length], train_index[-length:]
    ...
    for epoch in range(args.epochs):
        ...
        if args.validation == 1 and (epoch+1) in Epochs:
    ...
    if args.validation==0:
        ...
        min_loss_validation = min(Loss_validation)
        for e in range(len(Epochs)):
            if Loss_validation[e] == min_loss_validation:
                best_epoch = Epochs[e]
                break
