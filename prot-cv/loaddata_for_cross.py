...
def processWildFeaturesAndAdj(set_name ,nodes_per_graph):
    ...
    for i in range(len(wild_nodes_number)):
        ...
        if set_name == 'spike' or set_name[0:2]=='ac'or set_name[0] == 'd':
            file = '7kmb'
        else:
            file = ske[i][0]
        file = file + '.npy'
...
def getWildAndMutIndex(set_name):
    ...
    mut_nodes = np.load(set_name+'/nodes_number.npy')
    wild_features = np.load(set_name+'/wild_features.npy')
    wild_nodes = np.load(set_name+'/nodes_number.npy')

    w = np.load(set_name + '/mutation_site.npy', allow_pickle=True)
    m = np.load(set_name + '/mutation_site.npy', allow_pickle=True)
...
def standarize(mut_features, wild_features, mut_index_train, wild_index_train, wild, mutant, wild_test, mutant_test, nodes_per_graph):
    print('standarizing ...')
    ...
    stand = np.concatenate(( wild_features[wild_index_train], mut_features[mut_index_train]), 0)
    #standarize
    feature = stand
...
def get_features(features, nodes_number, indexes):
    ...
    for index in indexes:
        ...
        result.append(fea)
        i.extend([k for k in range(start, end)])
    return result, i
