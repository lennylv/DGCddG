...
mut_features = mut_features[:, 15:65]
wild_features = wild_features[:, 15:65]
...
for train_index, test_index in split_folds:
    ...
    # Y_pred = np.load('ab645.npy')
    Y_pred[test_index] = y_pred
    # np.save('ab645.npy', Y_pred)


print('pcc:', np.mean(val_pcc), ' rmse:', np.sqrt(mean_squared_error(Y_pred, np.load('ab645/itemlist.npy')[:,-1].astype(float))))
