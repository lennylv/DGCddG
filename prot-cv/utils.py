...
def processAdj(adj, nodes, this_nodes):
    adj = np.array(adj)

    b_adj = np.empty((nodes, nodes))
    b_adj[:this_nodes, :this_nodes] = adj
    # 
    adj = sp.coo_matrix(b_adj)
...
def getWildAdjAndFeature(item, scaler, nodes):
    structure = item[0]
    mut_chain = item[1]
    mut_id = int(item[3])
    
    adj_dis = '1131set/ordinary_adj/'+structure+'.npy' 
    adj = np.load(adj_dis, allow_pickle=True).tolist().todense()
    features_dis = '1131set/ordinary_features/'+structure+'.npy' 
    features = np.load(features_dis)
    this_nodes = len(features)
    for index in range(len(features)):
        if features[index][24]==mut_chain:
            features[index][24] = '1'
        else:
            features[index][24] = '0'
    #standardize
    ...
    feat = np.empty((nodes, features.shape[1]))
    feat[:this_nodes, :] = features.todense()
...
def calcPearson(x,y):
    x_mean,y_mean = calcMean(x,y)	#mean
    n = len(x)
    sumTop = 0.0
    sumBottom = 0.0
    x_pow = 0.0
