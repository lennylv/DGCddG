...
train_x = np.concatenate((train_x1[:, one_idx], train_x2[:, environ_idx], train_x3[:,normal_idx], train_x4[:, select_idx]), 1)
...
# np.save('au/train_aux.npy',train_x)
train_x = np.concatenate((train_x, train_x5), 1)

# train_y = train_y[train_singleidx]
# train_x = train_x[train_singleidx]
...
test_x4 = np.load(set_name+'/dssp.npy')
test_x5 = np.load('250epoch_model_graph_features/X_gbdt_'+set_name+'.npy')[:,-128:][:, importance]

test_x = np.concatenate((test_x1[:, one_idx], test_x2[:, environ_idx], test_x3[:, normal_idx], test_x4[:, select_idx]), 1)
for index in test_multiidx:
    item = test_list[index]
    numbers = len(item[1].split('_')[:-1])
    test_x[index] /= numbers

# np.save('au/test1_aux.npy',test_x)
test_x = np.concatenate((test_x, test_x5), 1)

##test_y = test_y[test_singleidx]
##test_x = test_x[test_singleidx]
##test_x = test_x5
...
print('loss:', RMSD)
print('kandell:', kan[0])
##print('alpha:',preds[2],'alpha*:',preds[-2],'delta:',preds[-1])
##print('beta:',preds[-3],'gamma:',preds[-4],'delta*:',preds[-5])
##np.save('prediction/'+set_name+'.npy',preds)
